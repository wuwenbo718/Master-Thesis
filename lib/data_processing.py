...
### Lower version for checking faster version
...
### Slower version for checking faster version
def compute_MDF_L(data):
    """
    ...
    for i in range(N):
        for j in range(M):
            freqs, power=signal.periodogram(data[i,:,j], 1e3)
            total = cumtrapz(power,freqs,initial=0)
            w=np.where(total>=(total[-1]/2))[0][0]
            feature[i,j] = freqs[w]
...
def compute_mDWT_L(data,wavelet='db7',level=3):
    """
...
def compute_mDWT_pd(data,wavelet='db7',level=3):
    ...
    """
    feature = compute_mDWT(data,wavelet,level)
