...
class WaveNetClassifier():
    ...
    def construct_model(self):    
        ...
        out = Conv1D(self.n_filters, 7, strides = 1, padding='same', name='conv_5ms', activation = 'relu')(out)
        out = AveragePooling1D(7, padding='same', name='downsample_to_200Hz')(out)
        if self.time_distributed:
            target_kernel_size = (int) (self.input_shape[0] / 7 / self.output_shape[0]) # prev_len / x = target_len => x = prev_len / target_len
            ...
            out = Conv1D(self.n_filters, 10, padding='same', activation='relu', name='conv_500ms')(out)
            out = Conv1D(self.output_shape[0], 10, padding='same', activation='relu', name='conv_500ms_target_shape')(out)
            out = AveragePooling1D(10, padding='same',name = 'downsample_to_2Hz')(out)
            out = Conv1D(self.output_shape[0], (int) (self.input_shape[0] / 8), padding='same', name='final_conv')(out)
            out = AveragePooling1D((int) (self.input_shape[0] / 8), name='final_pooling')(out)
